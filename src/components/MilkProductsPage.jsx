    ...
    const fetchMilkProducts = async () => {
      try {
        const response = await apiRequest(
          getApiUrl(API_ENDPOINTS.GET_MILK_PRODUCTS)
        );
        setMilkProducts(response.products || []);
      } catch (err) {
        setError("Failed to fetch milk products");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <PageHeader
        ...
        subtitle="Pure and fresh dairy products straight from local farms"
        icon="🥛"
        showSearch={true}
        searchQuery={searchQuery}
        onSearchChange={(e) => setSearchQuery(e.target.value)}
        searchPlaceholder="Search for dairy products..."
      />

      <section className="py-12 sm:py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          {loading && <LoadingSpinner message="Loading fresh dairy products..." />}
          ...
          >
            {filteredMilkProducts.length > 0 ? (
              filteredMilkProducts.map((product, index) => (
                <ProductCard
                  key={product._id}
                  product={product}
              ...
              !loading && (
                ...
                >
                  ...
                  <h3 className="text-2xl font-bold text-gray-600 mb-2">No dairy products found</h3>
