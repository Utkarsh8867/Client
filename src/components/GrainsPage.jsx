...
const GrainsPage = () => {
    ...
    const fetchGrains = async () => {
      try {
        const response = await apiRequest(
          getApiUrl(API_ENDPOINTS.GET_GRAINS)
        );
        setGrains(response.products || []);
      } catch (err) {
        setError("Failed to fetch grains");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <PageHeader
        title="🌾 Premium Grains Market"
        subtitle="Nutritious whole grains and cereals for a healthy lifestyle"
        icon="🌾"
        showSearch={true}
        searchQuery={searchQuery}
        onSearchChange={(e) => setSearchQuery(e.target.value)}
        searchPlaceholder="Search for grains..."
      />

      <section className="py-12 sm:py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          {loading && <LoadingSpinner message="Loading premium grains..." />}
            ...
            {filteredGrains.length > 0 ? (
              ...
              !loading && (
                ...
                >
                  ...
                  <h3 className="text-2xl font-bold text-gray-600 mb-2">No grains found</h3>
